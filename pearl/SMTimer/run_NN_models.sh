...
then
    # a smaller dataset for fast get started, change learning rate because the data size change.
     python train.py --data_source data/example/single_test --input data/example/pad_feature --model lstm --time_selection z3 --threshold 200 --regression --lr 0.5

    python train.py --input data/example/pad_feature --load_file e_pad_feature_l_z_r_200 --model lstm --time_selection z3 --regression

    python simulation.py --model_name lstm --load_file checkpoints/e_pad_feature_l_z_r_200.pkl --test_directory data/example/single_test/arch --time_selection adjust --regression
else
