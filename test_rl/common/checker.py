...
def reward_1(sample_index, holder, lambda_holder_eval, lambda_new_ice):
    ct = 0
    s = 0
    score = lambda_holder_eval('RL:')
    if 'RL:' in holder.ce_per_key:
        ct += len(holder.ce_per_key['RL:'].ce_list)
        s += 0.99

    stat_counter.add(sample_index, 'actual_z3')
    # otherwise, call the old reward
    return reward_0(holder, lambda_holder_eval, lambda_new_ice, score)
...
def boogie_result(g, expr_root):
    ...
    if cmd_args.only_use_z3:
        if cmd_args.single_sample is not None:
            lambda_new_ice = get_z3_ice( g.db.ordered_pre_post[g.sample_index], expr_root)
        else:
            lambda_new_ice = get_z3_ice( g.vc_list, expr_root)
    else:
        if cmd_args.single_sample is not None:
